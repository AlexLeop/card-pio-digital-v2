...
serve(async (req) => {
  ...
  try {
    ...
    if (orderId) {
      ...
      if (!orderError && orderData) {
        ...
        if (!storeError && storeData?.mercado_pago_access_token) {
          ...
          console.log('Store credentials found')
        }
      }
    }
    ...
    console.log('Creating card payment with data:', JSON.stringify({
      ...paymentData,
      payer: {
        ...paymentData.payer,
        identification: {
    ...
    console.log('Card payment processed:', payment.id, 'status:', payment.status)
