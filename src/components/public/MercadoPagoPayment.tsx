...
const MercadoPagoPayment: React.FC<MercadoPagoPaymentProps> = ({
  ...
  // Adicione este useEffect junto com os outros useEffects
  useEffect(() => {
    ...
    if (paymentMethod === 'credit_card' && mp && !loading) {
      ...
      const timer = setTimeout(() => {
        console.log('Initializing card payment from useEffect');
  ...
  const loadMercadoPago = async () => {
    try {
      console.log('Loading MercadoPago for payment method:', paymentMethod);
      ...
      console.log('Public key found, loading SDK...');
  ...
  const initializeMercadoPago = (publicKey: string) => {
    try {
      console.log('Initializing MercadoPago with key:', publicKey);
  ...
  const createPixPayment = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        console.error('PIX payment error response:', errorText);
  ...
  const createCardPayment = async (mercadopago: any) => {
    try {
      // Não ative o loading aqui, pois isso esconderia o container
      // setLoading(true); - Remova esta linha
      ...
      const findContainer = async (attempts = 0, maxAttempts = 5): Promise<HTMLElement | null> => {
        ...
        if (container) {
          console.log(`Container found on attempt ${attempts + 1}`);
          return container;
        }
      ...
      if (!cardFormContainer) {
        console.error('Card form container not found after multiple attempts');
        setErrorMessage('Erro ao encontrar o container do formulário. Tente novamente.');
        return;
      }
      
      console.log('Card form container found, initializing...', cardFormContainer);
      ...
      const settings = {
        ...
        callbacks: {
          onReady: () => {
            ...
            // Não desative o loading aqui, pois ele já deve estar desativado
            // setLoading(false); - Remova esta linha
          },
